...
public class PlayerPointsAPI {
    ...
    public boolean consume(String playerId, int amount , String info) {
        if(playerId != null && amount> 0){
            return plugin.getModuleForClass(StorageHandler.class).consumePoints(playerId , amount , info);
        }
        return false;
    }
    ...
    public boolean take(String playerName, int amount) {
    	...
    	if(playerName != null) {
            ...
            if(success){
                consume(playerName , amount , "unknow");
            }
    	}
    	return success;
    }

    public boolean take(String playerName, int amount , String info) {
        boolean success = false;
        if(playerName != null) {
            success = take(plugin.translateNameToUUID(playerName), amount);
            if(success) {
                consume(playerName, amount, info);
            }
        }
        return success;
    }
